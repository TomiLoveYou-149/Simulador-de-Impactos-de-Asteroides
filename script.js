...
function simulateImpact() {
  ...
  craterDiameterGlobal = craterDiameter; // Guardamos globalmente para TcA
...
// Dibujar TcA con escala realista y mínimo/máximo
function drawImpact() {
  ctx.clearRect(0,0,canvas.width,canvas.height);
  ctx.drawImage(worldMap,0,0,canvas.width,canvas.height);

  if(craterDiameterGlobal <= 0) return; // No dibujar si cráter 0

  // Escala realista
  const minPx = 20;
  const maxPx = 400;
  let radius = craterDiameterGlobal / 50; // Ajustar divisor para tamaño visual
...
// Movible manteniendo mouse pulsado
